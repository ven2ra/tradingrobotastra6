...
const hints: Record<string, string> = {
  step_unit: "В чём измеряется шаг сетки: проценты от цены, ATR или фиксированные пункты.",
  ...
  anchor: "От чего отсчитывать сетку: от последней цены или от зафиксированного значения.",
  ...
  outside_flat: "Что делать, если рынок вышел из флэта: отменить неисполненные входы или просто запретить новые.",
  lookback: "Число последних баров, по которым ищутся максимум и минимум для пробоя (канал Дончиана).",
  ma: "Тип скользящей средней, используемой как фильтр направления тренда.",
  ma_period: "Период скользящей средней в барах.",
  adx_min: "Минимальное значение индикатора ADX, при котором тренд считается достаточно сильным для входа.",
  pyramiding: "Разрешить ли докупать в позицию по мере движения цены в вашу сторону.",
  enabled: "Включает или выключает этот блок правил.",
  max_additions: "Максимальное число дополнительных входов сверх первого.",
  add_every_atr: "Через сколько ATR движения цены добавляется следующая часть позиции.",
  range_bars: "Число баров, за которые формируется диапазон перед пробоем.",
  min_volume_ratio: "Во сколько раз объём на пробое должен превышать средний, чтобы сигнал считался подтверждённым.",
  cancel_on_reentry: "Отменять неисполненную заявку, если цена вернулась обратно в диапазон.",
  session_from: "Время начала окна, в которое стратегии разрешено работать (МСК).",
  session_to: "Время конца окна, в которое стратегии разрешено работать (МСК).",
  confirmation_bars: "Сколько баров подряд должны подтверждать сигнал, прежде чем он считается действительным.",
  ...
  deviation_unit: "В чём измеряется отклонение от средней: в процентах или в стандартных отклонениях.",
  deviation: "Порог отклонения цены от средней, при котором возникает сигнал на возврат.",
  rsi_low: "Уровень индикатора RSI, ниже которого рынок считается перепроданным.",
  rsi_high: "Уровень индикатора RSI, выше которого рынок считается перекупленным.",
  extremum_bars: "Число баров, по которым ищется локальный экстремум для размещения стопа.",
  stop_buffer_points: "Дополнительный отступ стопа за найденным экстремумом, в пунктах цены.",
  compression_bars: "Число баров, по которым оценивается сжатие волатильности перед импульсом.",
  max_atr_ratio: "Максимальное отношение текущего ATR к среднему, при котором рынок ещё считается сжатым.",
  min_impulse_atr: "Минимальный размер импульсного движения в единицах ATR, чтобы считать его сигналом.",
  time_stop_bars: "Через сколько баров позиция закрывается по времени, если цель не достигнута.",
  events: "Типы событий, перед которыми действует запрет на вход (дивиденды, купон, оферта, размещение).",
  block_hours: "За сколько часов до события запрещается открывать новые позиции.",
  ...
  missing_calendar: "Что делать, если календарь событий недоступен: блокировать входы или пропустить проверку.",
  min_ytm: "Минимальная доходность к погашению, при которой облигация проходит отбор.",
  max_duration: "Максимальная дюрация облигации в годах, допустимая для отбора.",
  max_rating_rank: "Максимально допустимый (наихудший) ранг кредитного рейтинга бумаги.",
  min_daily_turnover_rub: "Минимальный дневной оборот бумаги в рублях — отсекает неликвидные выпуски.",
  max_spread_bps: "Максимально допустимый спред bid/ask в базисных пунктах (1 bps = 0,01%).",
  signal: "Способ расчёта целевой доходности для сравнения со спредом.",
  spread_bps: "Пороговый спред к кривой ОФЗ в базисных пунктах, при котором возникает сигнал.",
  offer_days: "За сколько дней до оферты бумага исключается из отбора.",
  dirty_anomaly_points: "Порог аномалии полной (грязной) цены в пунктах, отсекающий некорректные котировки.",
  ...
  yield_basis: "На основании чего считается доходность для сравнения: к погашению, к оферте или худший сценарий.",
  shock: "Действие при рыночном режиме SHOCK: отменять входы или полностью блокировать торговлю.",
  daily_stop: "Действие при срабатывании дневного лимита убытка.",
  imoex_drop_pct: "Падение индекса IMOEX в процентах, при котором активируется защита.",
  cancel_entries: "Отменять ли неисполненные заявки на вход при срабатывании защиты.",
  allow_only_reduction: "Разрешать только сделки, уменьшающие риск (закрытие или сокращение позиций).",
  ...
  activation_pct: "После какого движения цены (в процентах) трейлинг-стоп начинает подтягиваться.",
  breakeven_pct: "Движение цены в процентах, после которого стоп переносится в безубыток.",
  yield_bps: "Изменение доходности в базисных пунктах, которое запускает стоп по облигации.",
  trade_risk_rub: "Максимальная сумма риска на одну сделку в рублях.",
  max_weight_pct: "Максимальная доля одной бумаги в портфеле, в процентах от капитала.",
  ...
  daily_loss_rub: "Дневной лимит убытка в рублях. При достижении новые входы блокируются до следующей сессии.",
  weekly_drawdown_pct: "Максимально допустимая просадка за неделю в процентах от пикового значения капитала.",
  cooldown_seconds: "Пауза в секундах перед новыми входами после срабатывания стопа.",
  stale_seconds: "Через сколько секунд котировка считается устаревшей и перестаёт использоваться для решений.",
  fee_bps: "Комиссия брокера и биржи в базисных пунктах, учитываемая в расчёте P/L.",
  slippage_bps: "Запас на проскальзывание цены исполнения, в базисных пунктах.",
  event_block_hours: "За сколько часов до события риск-политика запрещает новый вход.",
  event_close_hours: "За сколько часов до события риск-политика принудительно закрывает позиции.",
  imoex_stop_pct: "Падение индекса IMOEX в процентах, при котором риск-политика останавливает торговлю целиком.",
};
